const { readStepEntries, writeStepEntries } = require("./lib/steps-store");
const { formatCreatedAt, getNextId, keyFromEvent, normalizePayload } = require("./lib/steps-utils");

const STEPS_KEY = process.env.STEPS_KEY || process.env.STEP_COUNT_KEY || process.env.SCREEN_TIME_KEY || process.env.CSV_KEY || "DIN_KEY";
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const entry = {
      id: nextId,
      date,
      total_steps: normalized.value.totalSteps,
    ...
    await writeStepEntries(event, entries);
    ...
    console.error("Kunne ikke lagre steg:", err);
    return json(500, { error: "Kunne ikke lagre steg" });
  }
};
